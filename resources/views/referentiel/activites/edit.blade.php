<x-app-layout :title="'Modifier activité — ' . $activite->libelle">

<x-page-header titre="Modifier : {{ $activite->libelle }}" />

<form method="POST" action="{{ route('referentiel.activites.update', $activite) }}" novalidate>
    @csrf @method('PUT')
                ...
                <div class="col-md-4">
                    ...
                    <select name="secteur_activite_id"
                        ...
                        @foreach ($secteurs as $s)
                            <option value="{{ $s->id }}"
                                    {{ old('secteur_activite_id', $activite->secteur_activite_id) == $s->id ? 'selected' : '' }}>
                                {{ $s->libelle }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-4">
                    ...
                    <select name="categorie_activite_id"
                        ...
                        @foreach ($categories as $c)
                            <option value="{{ $c->id }}"
                                    {{ old('categorie_activite_id', $activite->categorie_activite_id) == $c->id ? 'selected' : '' }}>
                                {{ $c->libelle }}
                            </option>
                        @endforeach
                    </select>
